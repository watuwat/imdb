...
var _ Lexer = &StreamLexer{}

//StreamLexer lexer struct
...
func (l *StreamLexer) PeekNth(n int) rune {
	var val rune
...
func (l *StreamLexer) Accept(valid string) bool {
	val := l.Next()
...
func (l *StreamLexer) AcceptRun(valid string) {
	val := l.Next()
...
func (l *StreamLexer) AcceptRunUntil(notValid string) {
	val := l.Next()
...
//NewStreamLexer creates a stream lexer object with all the proper internal variables
func NewStreamLexer(input io.Reader, bufferSize int) *StreamLexer {
	return &StreamLexer{
		input:       input,
		buffer:      nil,
